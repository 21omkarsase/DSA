// Sum Tree
// MediumAccuracy: 37.17%Submissions: 152K+Points: 4
...
{
    public:
     int sumTreeOrNot(Node* root,bool&ans){
        if(!root)return 0;
        if(!root->left and !root->right)return root->data;
        
        int leftSum,rightSum;
        leftSum=sumTreeOrNot(root->left,ans);
        rightSum=sumTreeOrNot(root->right,ans);
