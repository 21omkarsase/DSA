    ...
    vector<long long> countBlackBlocks(int n, int m, vector<vector<int>>& coordinates) {
        ...
        for (int row = 1; row < n; row++) {
            for (int col = 1; col < m; col++) {
                ...
                int totalBlacks = dp[row][col];

                if (startCol - 1 >= 0 && startRow - 1 >= 0) {
                    totalBlacks -= (dp[row][startCol - 1] + dp[startRow - 1][col]);
                    totalBlacks += dp[startRow - 1][startCol - 1];
                } else if (startCol - 1 >= 0) {
                    totalBlacks -= dp[row][startCol - 1];
                } else if (startRow - 1 >= 0){
                    totalBlacks -= dp[startRow - 1][col];
                }
