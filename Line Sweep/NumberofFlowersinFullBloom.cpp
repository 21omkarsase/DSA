...
public:
    vector<int> fullBloomFlowers(vector<vector<int>>& flowers, vector<int>& people) {
        ...
        for (int idx = 0; idx < n; idx++) {
            tracker[flowers[idx][0]]++;
            tracker[flowers[idx][1] + 1]--;
        }

        vector<int> bloomFlowers(m, 0);
        ...
        for (int idx = 0; idx < m; idx++) {
            auto it = tracker.upper_bound(people[idx]);
            auto prevIt = prev(it);
            
            bloomFlowers[idx] = it == prevIt ? 0 : prevIt->second;
        }
